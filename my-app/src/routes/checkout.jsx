import {
    Modal,
    ModalOverlay,
    ModalContent,
    ModalHeader,
    ModalFooter,
    ModalBody,
    ModalCloseButton,
    useDisclosure,
    Button,
    FormControl,
    FormLabel,
    Input,
    Image,
    HStack,
    PinInput,
    PinInputField,
    Spinner,
    Box,
    Text,
    Img,
} from '@chakra-ui/react'
import React, { useState } from 'react'
import { useRef } from 'react'
import { Navigate, useNavigate } from 'react-router-dom'
import style from "./check.module.css"
const Pin=()=>{
    return(
        <HStack>
        <PinInput>
            <PinInputField border="1px solid purple" />
            <PinInputField border="1px solid purple" />
            <PinInputField border="1px solid purple" />
            <PinInputField border="1px solid purple" />
        </PinInput>  
    </HStack>
    )
}

export function SizeExample({ state, setState }) {
    const navigate = useNavigate()
    const { isOpen, onOpen, onClose } = useDisclosure()
    const [loading ,setLoading] = useState(true)
    const [Sloading ,setSLoading] = useState(false)
    const [isPaying ,setIsPaying] = useState(false)

    const sizes = ['full']
    const handleClose = () => {
        setState(false)
    }
    const ref = useRef()
    const handleSubmit = () => {
        setSLoading(true)
            ref.current=setInterval(()=>{
                setLoading(false)
                setSLoading(false)
            },3000)
    }
    const handleDone=()=>{
        clearInterval(ref.current)
        setSLoading(true)
        setInterval(()=>{
            setSLoading(false)
            setIsPaying(true) 
        },2000) 

    }
    return (

        <>
            <Modal onClose={state} size={sizes} isOpen={state}>
                <ModalOverlay />
                <ModalContent textAlign="center" w="70%" className={style.check}>
                    <ModalHeader w="50%" margin="auto" margintop="10px">CheckOut Form
                        <Image src="data:image/svg+xml;base64," />

                    </ModalHeader>
                    <ModalCloseButton onClick={handleClose} />
                    <ModalBody w="50%" className={style.input}margin="auto">
                        <FormControl>
                            <FormLabel>First name</FormLabel>
                            <Input placeholder='First name' />
                        </FormControl>

                        <FormControl mt={4}>
                            <FormLabel>Card Number</FormLabel>
                            <Input placeholder='Card Number' />
                        </FormControl>
                        <FormControl mt={4}>
                            <FormLabel>Email</FormLabel>
                            <Input placeholder='Enter Your Email' />
                        </FormControl>
                        <FormControl mt={4}>
                            <FormLabel>CVV</FormLabel>
                            <Input placeholder='CVV' type="password" />
                        </FormControl>
                        <Button marginTop="10px" onClick={handleSubmit}>Proceed To Pay</Button>
                        <Box w="50%" margin="auto" marginTop="20px">
                        {Sloading ? <Spinner size="xl"/>: null}
                        </Box>
                        <Box w="30%" margin="auto" marginTop="20px">
                            
                        { loading ? null :   <Box><Text as="em">Enter Your Pin</Text><Pin/><Button onClick={handleDone} marginTop="20px" >Proceed</Button></Box> }
                        </Box>
                        {!isPaying ? null :<Box>
                            <Img src="https://i.gifer.com/origin/41/41340ab1a4529c7dd753f03268087e08.gif"/>
                        </Box>}
                        
                    </ModalBody>
                    <ModalFooter>
                        <Button onClick={handleClose}>Close</Button>
                    </ModalFooter>
                </ModalContent>
            </Modal>
        </>
    )
}
